...
function parseCVSS40(cvss40, skipPrefix = false) {
  ...
  const extractValueMetric = function(metric, cvssStr, constraint) {
    return extractValueMetricFromMap(metric, cvssStr, metricsMap, constraint);
  };
  const extractValueMetricOptional = function(metric, cvssStr, defaultValue, constraint) {
...
function extractValueMetricFromMap(metric, cvssStr, metrics, constraint) {
...
function extractValueMetricOptionalFromMap(metric, cvssStr, defaultValue, metrics, constraint) {
  if (metrics.has(`M${metric}`)) {
    const r = metrics.get(`M${metric}`);
    metrics.delete(metric);
    if (isOfType(r) && constraint.includes(r)) {
